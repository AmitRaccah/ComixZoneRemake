    ...
    void Update()
    {
        if (isWiping)
        {
            ...
            foreach (Renderer rend in renderers)
            {
                ...
                // SET THE CHARACTER OFFSET! This is what was missing!
                propertyBlock.SetFloat("_WipeOffset", transform.position.y);
            
                propertyBlock.SetFloat("_WipeMinY", minY - transform.position.y);  // Make relative!
                propertyBlock.SetFloat("_WipeMaxY", maxY - transform.position.y);  // Make relative!
                propertyBlock.SetFloat("_WipeTheshold", actualThreshold - transform.position.y);  // Make relative!
