    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                // Subtle rim lighting only - NO halftone on the object itself
                float rim = 1.0 - saturate(dot(i.viewDir, i.worldNormal));
                rim = pow(rim, _RimPower);
                float3 rimGlow = _GlowColor.rgb * rim * _GlowIntensity * 0.2; // Very subtle
                
                // Only add the rim glow to the object
                col.rgb += rimGlow;
