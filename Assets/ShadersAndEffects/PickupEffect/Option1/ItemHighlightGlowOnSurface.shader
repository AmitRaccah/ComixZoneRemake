Shader "Custom/ItemHighlightGlow"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // Base color from texture and tint
                ...
                // Halftone pattern on ground plane
                float2 worldPosXZ = i.worldPos.xz;
                float2 objectCenterXZ = mul(unity_ObjectToWorld, float4(0, 0, 0, 1)).xz;
                
                float halftone = HalftonePattern(worldPosXZ, objectCenterXZ);
                float3 halftoneGlow = _GlowColor.rgb * halftone * _GlowIntensity;
                
                // Combine
                col.rgb += rimGlow;
                col.rgb += halftoneGlow * 0.5;
        ...
        /*
        // Second pass - ground projection (halftone dots on ground)
        // NOTE: Commented out for initial testing - uncomment once main effect works
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                return fixed4(color, halftone * 0.8);
            }
            ENDCG
        }
        */
